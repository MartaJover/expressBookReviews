...
public_users.get('/isbn-async/:isbn', async (req, res) => {
  try {
    ...
    // Call the existing '/isbn/:isbn' endpoint on your server
    const response = await axios.get(`http://localhost:5001/isbn/${isbn}`);

    // Return the data received from the '/isbn/:isbn' endpoint
    return res.status(200).json(response.data);
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Error retrieving book by ISBN", error: error.message });
  }
});
...
public_users.get('/author-async/:author', async (req, res) => {
  try {
    ...
    // Call the existing '/author/:author' endpoint on your server
    const response = await axios.get(`http://localhost:5001/author/${author}`);

    // Return the data received from the '/author/:author' endpoint
    return res.status(200).json(response.data);
  } catch (error) {
    return res.status(500).json({ 
      message: "Error retrieving book(s) by author", 
      error: error.message 
    });
  }
});
...
public_users.get('/title-async/:title', async (req, res) => {
  try {
    ...
    // Call the existing '/title/:title' endpoint on your server
